-- Extends get_user_tree_data (migration 051) to also return this feature's
-- new tree_growth_score and tree_environment_preference — additive fields
-- only, every existing field/behavior is unchanged byte-for-byte.
...
declare
  ...
  v_streak_days int;
  v_tree_growth_score numeric;
  v_tree_environment_preference text;
begin
  ...
  select last_active_at, created_at, coalesce(streak_days, 0),
         coalesce(tree_growth_score, 0), tree_environment_preference
    into v_last_active_at, v_joined_at, v_streak_days,
         v_tree_growth_score, v_tree_environment_preference
  ...
  return jsonb_build_object(
    ...
    'treeGrowthScore', coalesce(v_tree_growth_score, 0),
    'treeEnvironmentPreference', v_tree_environment_preference
  );
end;
$$;
