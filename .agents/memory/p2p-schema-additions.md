...
## Two-tier Help system (added 2026-07-09)

New table `p2p_help_requests(id, user_id, tier crisis|struggling, category?, note?, status open|contacted|resolved, created_at)` with RLS (own insert/select; admin select/update via `p2p_is_admin()`). Trigger `p2p_notify_on_help_request()` fires on insert: for `tier='crisis'` it notifies users with a row in `p2p_admin_roles` where `role='crisis_responder'` (a dedicated role, deliberately NOT the general peer-guide/admin pool, since crisis routing must not depend on the peer-guide relationship being staffed/responsive); for `tier='struggling'` it notifies every profile with `p2p_profiles.role != 'student'` (mirrors `p2p_is_admin()`), not `p2p_admin_roles` — `p2p_admin_roles` is otherwise an unused/empty table, real role gating in this app lives on `p2p_profiles.role` (enum: student/peer_guide/church_leader/regional_admin/moderator/super_admin). No one currently holds `crisis_responder` — an admin must insert a row into `p2p_admin_roles` (role='crisis_responder') for a specific user before crisis notifications have any recipient.
