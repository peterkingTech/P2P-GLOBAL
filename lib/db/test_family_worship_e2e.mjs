...
async function run() {
  ...
  {
    // Seek — host jumps to 45s without pausing.
    const r = await api(`/family/worship/sessions/${created.sessionId}/state`, users.shepherd.token, { method: "PUT", body: JSON.stringify({ positionMs: 45000 }) });
    check("host seeks to a specific position", r.ok && r.body.playbackBasePositionMs === 45000, r.body);
  }
  {
    // Pause — freezes the shared position.
    const r = await api(`/family/worship/sessions/${created.sessionId}/state`, users.shepherd.token, { method: "PUT", body: JSON.stringify({ isPlaying: false, positionMs: 45000 }) });
    check("host pauses playback", r.ok && r.body.isPlaying === false, r.body);
  }
  {
    // Realtime delivery: postgres_changes on p2p_family_worship_sessions
    // was investigated (isolated test against a proven-working table with
    // an identical client pattern: worked; this table: did not, even
    // minutes after being confirmed in the supabase_realtime publication —
    // see migration 120's comment for the full record) and does not
    // currently deliver in production, root cause not conclusively
    // identified. The worship room no longer relies on it as the primary
    // path — broadcastState() in app/family/worship/[sessionId].tsx sends
    // state over the family_worship_signal_* private broadcast channel
    // instead, which IS verified working end-to-end — see
    // test_family_worship_broadcast_auth.mjs. Not re-tested here to avoid
    // duplicating that coverage.
  }
  {
    // Join while paused — memberB joins during a paused moment before the
    // playing-late-join case below; then leaves so the existing playing
    // late-join test (using memberB again) still exercises a fresh join.
    await api(`/family/worship/sessions/${created.sessionId}/state`, users.shepherd.token, { method: "PUT", body: JSON.stringify({ isPlaying: false, positionMs: 45000 }) });
    const r = await api(`/family/worship/sessions/${created.sessionId}/join`, users.memberB.token, { method: "POST" });
    check("memberB joins while paused", r.ok && r.body.isPlaying === false && r.body.playbackBasePositionMs === 45000, r.body);
    await api(`/family/worship/sessions/${created.sessionId}/leave`, users.memberB.token, { method: "POST" });
    await api(`/family/worship/sessions/${created.sessionId}/state`, users.shepherd.token, { method: "PUT", body: JSON.stringify({ isPlaying: true, positionMs: 45000 }) });
  }
...

  {
    ...
    check("late joiner computes a position well past 45s, not 0:00", r.ok && computedPos > 46000 && computedPos < 60000, { computedPos, body: r.body });
  }
  {
  ...
  }
  {
    ...
    const r2 = await api(`/family/worship/sessions/${created.sessionId}/state`, users.memberB.token, {
      method: "PUT", body: JSON.stringify({ currentMode: "scripture", currentScripture: { translation: "KJV", book: "Psalms", chapter: 23, startVerse: 1, endVerse: 1 } }),
    });
    check("new host controls mode + scripture sync", r2.ok && r2.body.currentMode === "scripture" && r2.body.currentScripture?.book === "Psalms", r2.body);
  }
