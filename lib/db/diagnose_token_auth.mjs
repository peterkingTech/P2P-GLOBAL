...
// Get the REAL deployed channel name from the live endpoint, not a local
// reimplementation, so this test reflects the actual current server code.
const peerChannelRes = await fetch("https://workspaceapi-server-production-6d56.up.railway.app/api/calls/peer-channel", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ currentUserId: TEST3_ID, otherUserId: NOX_ID }),
});
const { channelName } = await peerChannelRes.json();
console.log("Channel name from live /calls/peer-channel:", channelName, `(${channelName.length} bytes)`);
