...
function MarketingAdminDashboard() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <QueueLinkCard label="Compose" sub="Message a user as P2P Global" path="/admin/send-message" icon="send" />
          ...
          {tab === "management" && (
            <ScrollView contentContainerStyle={styles.content}>
              ...
              <TouchableOpacity style={styles.queueCard} onPress={() => router.push("/admin/send-message" as any)}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.queueLabel}>Compose</Text>
...
export default function AdminHome() {
  ...
  switch (role) {
    ...
    case "admin_help": return <RoleQueueDashboard role="admin_help" queueLabel="Help Requests" queueSub="Crisis and struggling-tier cases" queuePath="/admin/help-requests" queueIcon="medkit" extraLinks={[{ label: "Email Inbox", sub: "Contact P2P Global messages", path: "/admin/email-inbox", icon: "mail" }, { label: "Compose", sub: "Message a user as P2P Global", path: "/admin/send-message", icon: "send" }]} />;
