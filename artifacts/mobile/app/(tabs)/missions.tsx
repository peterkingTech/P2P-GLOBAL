  ...
  return (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.cardName}>{item.isAnonymous ? "Anonymous" : item.userName}</Text>
          <Text style={styles.cardMeta}>{timeAgo(item.createdAt)} · Kingdom Win</Text>
        </View>
      </View>
      <Text style={styles.cardBody}>{item.body}</Text>
      <View style={styles.cardFooter}>
        <Ionicons name="hand-left-outline" size={13} color={colors.textMuted} />
        <Text style={styles.cardFooterText}>{item.prayingCount} praying · {item.amenCount} amen</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 13, color: colors.textMuted, marginTop: 4, marginBottom: 16, fontFamily: "Inter_400Regular" },
  statsRow: {
    flexDirection: "row", alignItems: "center",
    backgroundColor: colors.card, borderRadius: 14, borderWidth: 1, borderColor: colors.borderBeige, padding: 14,
  },
  statBox: { flex: 1, alignItems: "center" },
  statNum: { fontSize: 16, fontWeight: "700", color: colors.primaryGreen, fontFamily: "Inter_700Bold" },
  statLabel: { fontSize: 10, color: colors.textMuted, marginTop: 2, fontFamily: "Inter_400Regular", textAlign: "center" },
  statDivider: { width: 1, height: 32, backgroundColor: colors.borderBeige },
  sectionTitle: { fontSize: 16, fontWeight: "700", color: colors.textDark, fontFamily: "Inter_700Bold", paddingHorizontal: 20, paddingTop: 16, paddingBottom: 4 },
  loading: { alignItems: "center", paddingVertical: 40 },
  list: { paddingHorizontal: 20, paddingTop: 10 },
  empty: { alignItems: "center", paddingTop: 40, paddingHorizontal: 20, gap: 10 },
  emptyTitle: { fontSize: 16, fontWeight: "600", color: colors.textDark, fontFamily: "Inter_600SemiBold", textAlign: "center" },
  emptyText: { fontSize: 13, color: colors.textMuted, fontFamily: "Inter_400Regular", textAlign: "center", lineHeight: 20 },
  card: {
    backgroundColor: colors.card, borderRadius: 16, borderWidth: 1, borderColor: "rgba(224,164,65,0.3)",
  ...
  cardName: { fontSize: 13, fontWeight: "600", color: colors.textDark, fontFamily: "Inter_600SemiBold" },
  cardMeta: { fontSize: 11, color: colors.textMuted, fontFamily: "Inter_400Regular", marginTop: 1 },
  cardBody: { fontSize: 14, color: colors.textMid, lineHeight: 21, fontFamily: "Inter_400Regular", marginBottom: 10 },
  cardFooter: { flexDirection: "row", alignItems: "center", gap: 6 },
  cardFooterText: { fontSize: 12, color: colors.textMuted, fontFamily: "Inter_400Regular" },
});
